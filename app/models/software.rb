class Software
  ...
  #associations
  has_many :software_instances, autosave: true, dependent: :destroy
  has_many :relationships, as: :item, dependent: :destroy
  #slug
  slug :name do |software|
    software.name
               .downcase
               .gsub(/[^a-z0-9_-]/,"-")
               .gsub(/--+/, "-")
  ...
  validates_associated :software_instances

  accepts_nested_attributes_for :software_instances, allow_destroy: true,
  ...
  def sorted_software_instances
    ary = software_instances
  ...
  def dokuwiki_pages
    keywords = [name] + software_instances.map(&:software_urls).flatten.map(&:url)
