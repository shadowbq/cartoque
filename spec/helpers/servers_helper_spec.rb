...
describe ServersHelper do
  it "renders options for location filter" do
    ...
    expect(text).to have_selector :css, "option[value='']", ""
    expect(text).to have_selector :css, "option[value='site-#{@site1.id}']", @site1.name
    expect(text).to have_selector :css, "option[value='rack-#{@rack1.id}']", "&nbsp; #{@rack1}"
  end
end
